...
async function getModelSession() {
    if (!modelSessionPromise) {
        modelSessionPromise = (async () => {
            ...
            const modelUrl = new URL("./companella/dan_model.onnx", import.meta.url);
            const modelSource = (typeof process !== "undefined" && process?.versions?.node)
                ? new Uint8Array(await readFile(modelUrl))
                : modelUrl.toString();
