...
internal static class EtternaRuntime
{
    ...
    private sealed class MinaCalcModule
    {
        ...
        {
            string wasmFileName = GetWasmFileName(version);
            _engine = new Engine();
            using Stream wasmStream = AnalyzerResources.OpenStream(wasmFileName);
            _module = Module.FromStream(_engine, wasmFileName, wasmStream);
